...
| Current phase | **Phase 6 IN PROGRESS — preflight only.** Cutover **blocked**: a stranded-`queued` recovery defect was found and fixed (needs deploy), Heroku ACM is disabled, and no Cloudflare Zone·DNS token exists. **No production change made** |
