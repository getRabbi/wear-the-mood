...
| Current phase | **Phase 6 IN PROGRESS — candidates only.** **All 9 Azure Jobs on CI-built images** (8 orchestrator → `b9817f63`, rembg → `1749dea3`; both proven to run) + migration `0045` applied + Heroku **staging** released (v37, `18bb4ac`). CI image pipeline **unblocked** (GHCR packages linked + Write; `migration-build` green, all 3 digests verified). Heroku **prod still on `17a3a8c`** — PR #1 merged, `production` gate live (getRabbi reviewer) and approval worked, but the CI deploy (`29723080914`) **failed at the Heroku login step: `HEROKU_API_KEY` + `HEROKU_EMAIL` secrets are unset** (CI has only `LOADTEST_USERS_JSON`). Build/push/release were skipped — prod untouched, still v4, `/readyz` 200. Set the two secrets, then re-run (re-approves at the gate). Cutover also blocked on ACM + Cloudflare token. **Production traffic/DNS untouched** |
