...
| Current phase | **Phase 6 IN PROGRESS — candidates only.** **All 9 Azure Jobs on CI-built images** (8 orchestrator → `b9817f63`, rembg → `1749dea3`; both proven to run) + migration `0045` applied + Heroku **staging** released (v37, `18bb4ac`). CI image pipeline **unblocked** (GHCR packages linked + Write; `migration-build` green, all 3 digests verified). Heroku **prod still on `17a3a8c`** — `HEROKU_API_KEY` + `HEROKU_EMAIL` now set as **production-environment** secrets (dedicated Heroku authorization `70a12062…`, token never logged); deploy `29723080914` re-run and **re-paused at the `production` approval gate** awaiting getRabbi. Nothing built/pushed/released; prod still v4, `/readyz` 200. Cutover also blocked on ACM + Cloudflare token. **Production traffic/DNS untouched** |
