...
| Current phase | **Phase 6 IN PROGRESS — candidates only.** Recovery fix LIVE on Azure (8 orchestrator jobs now on the **CI-built** digest `b9817f63`, verified run) + migration `0045` applied + Heroku **staging** released (v37, `18bb4ac`). CI image pipeline **unblocked** (GHCR packages linked + Write; `migration-build` green, all 3 digests verified). Heroku **prod HELD on stale `17a3a8c`** — release it **through CI** (PR #1 + a real `production` gate), not by hand. Cutover also blocked on ACM + Cloudflare token. **Production traffic/DNS untouched** |
