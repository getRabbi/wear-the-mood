...
| Field | Value |
|---|---|
| Working branch | `migration/heroku-azure` (fully merged into `main`; `main` is ahead by 26 commits) |
| Base commit (`origin/main`) | `98df3c359ff711d4949e27b7ac2de4528602829b` |
| Current phase | **Phase 7 — PRE-DELETION AUDIT COMPLETE, droplet NOT deleted.** Migration is **DONE**: API on Heroku `wtm-api-prod` v16 (`78e7040`), website on Cloudflare Pages `wtm-site`, admin on Heroku `wtm-admin`, all 9 workers/schedules on Azure Container Apps Jobs, DB on Supabase US, media writes on Cloudflare R2. The DigitalOcean droplet served **zero** requests from 2026-07-20T13:04:06Z onward and was proven unnecessary by a **61-minute controlled blackout on 2026-07-26 (18:47:58Z → 19:48:57Z)** with zero production impact. Its containers are **stopped and left stopped** (`restart=no`). Full evidence in `PHASE_7_PRE_DELETE_AUDIT.md`. Awaiting `AUTHORIZE DIGITALOCEAN DECOMMISSION`. |
| Last completed | Phase 6 — production cutover + 48h soak (elapsed 2026-07-22 12:35Z) |
| DigitalOcean role | **NONE — decommissioned in place.** All five containers stopped 2026-07-26T18:47:58Z with restart policies disabled. **No longer a rollback path** (see `ROLLBACK_RUNBOOK.md`). Droplet `577335646` awaits deletion. |
| ⚠ Known operational risk | **Heroku ACM certificate for `api.wearthemood.com` expires 2026-10-18.** Initial issuance only succeeded with the Cloudflare record grey-clouded; the proxy is now on, so automatic renewal (~30 days prior) may fail silently. Monitor `heroku certs:auto` from mid-September 2026. |
| Authoritative DB | **Supabase US `ghzabbceoaoertatkjyg` (us-east-1)** — Tokyo retained as cold backup (do NOT delete) |
| Next human approval phrase | **`AUTHORIZE DIGITALOCEAN DECOMMISSION`** — the only outstanding gate. It authorizes deleting droplet `577335646` **only**; every other DigitalOcean resource stays (see `PHASE_7_PRE_DELETE_AUDIT.md` §2B/§2C) |
...
| 6 | Production cutover + 48h soak | ✅ **complete 2026-07-20**, soak elapsed 2026-07-22 12:35Z | `APPROVED PHASE 6` |
| 7 | DigitalOcean decommission | 🔶 **pre-deletion audit complete 2026-07-26** (`PHASE_7_PRE_DELETE_AUDIT.md`); 61-min blackout passed; droplet dark and **not deleted** | `AUTHORIZE DIGITALOCEAN DECOMMISSION` |
...
- **Phase 6 (WEBSITE cut over to Cloudflare Pages — apex live, all checks pass)** — owner decisions applied: Heroku admin is the canonical admin entry point, the old `/mood-ops-console-7x9` apex route is **not** recreated, and legal URLs are the extensionless forms.
  - Deployed `deploy/site` (16 files, incl. the new `_redirects`) to **production branch `main`** → deployment `b8a9be7b`. **Verified in full on `wtm-site.pages.dev` BEFORE attaching the apex**, then attached `wearthemood.com` to the project (status `initializing`) while DNS still pointed at DO — no traffic moved by the attach.
  - **Apex flipped 13:02:51Z**: record `70724268…` `A 159.65.248.247 proxied` → **`CNAME wtm-site.pages.dev` proxied**. All eight checks passed **19 s** later; effectively no downtime (Cloudflare-proxied on both sides, only the origin changed).
  - **Verified live on the apex:** landing 200 · `/legal/privacy`, `/legal/terms`, `/legal/acceptable-use` 200 (extensionless) · `/delete-account` 200 · `/invite/` 200 · `.html` forms 308 → canonical extensionless · **`assetlinks.json` and `apple-app-site-association` both 200 `application/json` with 0 redirects** (App Links / Universal Links intact) · `/r/*` end-to-end **8/8** via the Pages rule → API → site, final 200 · HTTPS `CN=wearthemood.com`, Google Trust Services, valid to 2026-09-19.
  - **`api.wearthemood.com` untouched** — still Heroku (`via: 2.0 heroku-router`), 200.
  - **Propagation note:** for ~1 min after the flip some edge nodes still resolved to DO, so `/r/` briefly showed the old single-hop redirect. It settled on its own; 8/8 fresh codes now take the Pages path. Not a defect — worth expecting on any apex move.
  - **⚠ Soft-404 behaviour:** Pages has no `404.html`, so unknown paths return **200 with the landing page** rather than a real 404. Consequence: `/mood-ops-console-7x9` on the apex now serves the landing page — the admin is **not** exposed (the desired outcome), but it is a soft 404. Worth adding a `404.html` for SEO hygiene; not a launch blocker.
  - **Admin confirmed working** at the canonical Heroku entry point: `https://wtm-admin-aab1ebe5235d.herokuapp.com/mood-ops-console-7x9` → **307 → login**. (`/`, `/dashboard`, `/login` 404 by design — the console lives under its own basePath; an earlier note claiming `/dashboard` → 307 was stale.)
  - **Rollback (one call):** PUT apex record `70724268…` back to `{"type":"A","content":"159.65.248.247","proxied":true,"ttl":1}`. DO `api`+`caddy` remain running and still serve the full site.
