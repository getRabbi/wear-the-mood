...
| Current phase | **Phase 6 IN PROGRESS — candidates only.** Recovery fix LIVE on Azure (8 jobs on digest `2db2601f`, verified run) + migration `0045` applied + Heroku **staging** released (v37, `18bb4ac`). Heroku **prod HELD on stale `17a3a8c`** — owner's decision (2026-07-20) is to release it **through CI**, not by hand, to keep the `production` review gate. Path in `ROLLBACK_RUNBOOK.md` → "Releasing Heroku prod through CI". Cutover also blocked on ACM + Cloudflare token. **Production traffic/DNS untouched** |
