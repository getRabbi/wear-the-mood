...
| Current phase | **Phase 6 IN PROGRESS — candidates only.** **All 9 Azure Jobs on CI-built images** (8 orchestrator → `b9817f63`, rembg → `1749dea3`; both proven to run) + migration `0045` applied + Heroku **staging** released (v37, `18bb4ac`). CI image pipeline **unblocked** (GHCR packages linked + Write; `migration-build` green, all 3 digests verified). Heroku **prod RELEASED to v5** (`0851595`) via gated CI; workflow now stamps `GIT_SHA` (main synced, PR #2). **Heroku ACM enabled** (status `Failing — CDN not returning HTTP challenge`, expected until DNS moves). **⛔ Cutover BLOCKED: the provided `CLOUDFLARE_API_TOKEN` is scoped to the wrong account** (`Servicerabbi@gmail.com's Account`, sees **0 zones**) — it cannot read/edit the `wearthemood.com` DNS. **DO worker deliberately NOT stopped** (won't take the cutover's biggest production action for a cutover that can't complete). Need a correct Zone:Read+DNS:Edit token, then steps 4–6 run in one window. **Production traffic/DNS untouched; DO worker still running** |
