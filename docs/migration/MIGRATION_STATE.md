...
| Current phase | **Phase 6 IN PROGRESS — deploying the fix (candidates only).** Recovery fix LIVE on Azure (8 jobs on digest `2db2601f`, verified run) + migration `0045` applied + Heroku **staging** released (v37, `18bb4ac`). Heroku **prod NOT released** (bypasses the review gate — awaiting the owner's word). Cutover still blocked on ACM + Cloudflare token. **Production traffic/DNS untouched** |
