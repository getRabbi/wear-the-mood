...
| Current phase | **Phase 6 IN PROGRESS — candidates only.** **All 9 Azure Jobs on CI-built images** (8 orchestrator → `b9817f63`, rembg → `1749dea3`; both proven to run) + migration `0045` applied + Heroku **staging** released (v37, `18bb4ac`). CI image pipeline **unblocked** (GHCR packages linked + Write; `migration-build` green, all 3 digests verified). Heroku **prod still on `17a3a8c`** — PR #1 merged, `production` gate live (getRabbi reviewer), and a CI prod deploy (`migration-deploy` run `29723080914`) is **paused at the approval gate** awaiting getRabbi; nothing built/pushed/released yet. Cutover also blocked on ACM + Cloudflare token. **Production traffic/DNS untouched** |
...
- **Phase 6 (CI prod deploy triggered — paused at the gate)** — PR #1 was found still **open** (the owner believed it merged; it had not), so merged it (`migration-deploy.yml` now on `main`, HEAD `ff53a1f`). Verified the `production` environment now has **getRabbi** as a required reviewer (it did — `total_count` went 0→1). Dispatched `migration-deploy` `target=prod` against `--ref migration/heroku-azure` (run `29723080914`); it is **`waiting`** on the `production` approval gate — pending-deployments API confirms env=production, reviewer getRabbi, `current_user_can_approve=true`. **No step ran**: nothing built, pushed, or released; prod still v4 (`17a3a8c`). Stopped here per instruction — approval is the human's action. DNS untouched, nothing routed.
