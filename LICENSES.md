...
| camera | BSD-3-Clause ✅ commercial OK | in-use (0.11.2) | Official flutter.dev plugin. iOS/iPadOS Try-On person capture: a fresh **front-camera** still taken behind a live preview with a full-body guide, framing feedback and a 3-2-1 auto-capture — none of which the system `image_picker` sheet can carry, because it cannot be drawn on. Opened with `enableAudio: false`, so **no microphone permission is ever requested**. Pulls the endorsed federated implementations `camera_avfoundation`, `camera_android_camerax` and `camera_web`, all BSD-3-Clause |
