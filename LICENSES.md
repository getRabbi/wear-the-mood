...
| shared_preferences | BSD-3-Clause | in-use (2.5.5) | Non-sensitive Discover state: story seen versions, recent searches, recently viewed product ids (DISCOVER §6.4/§11). Never secrets — those stay in flutter_secure_storage |
| visibility_detector | BSD-3-Clause (Google) | in-use (0.4.0+2) | Discover impression analytics — 50% visible for 500ms before an event counts (DISCOVER §22) |
...
| image_picker | BSD-3-Clause | in-use (1.2.2) | Camera/gallery capture for wardrobe add, profile, community posts and giveaways (§8). **No longer the source of a Try-On person image on iOS/iPadOS** — see `camera` below |
| camera | BSD-3-Clause ✅ commercial OK | in-use (0.11.2) | Official flutter.dev plugin. iOS/iPadOS Try-On person capture: a fresh still taken behind a live in-app preview with a full-body guide, framing feedback, a 3-2-1 auto-capture on the **front** lens and a manual shutter on the **rear** one (for when somebody is helping) — none of which the system `image_picker` sheet can carry, because it cannot be drawn on, and it cannot switch lenses under our own guide either. Opened with `enableAudio: false`, so **no microphone permission is ever requested**. Pulls the endorsed federated implementations `camera_avfoundation`, `camera_android_camerax` and `camera_web`, all BSD-3-Clause |
...
| _dev/ops:_ markdown | BSD-3-Clause | in-use | Render legal/*.md → static HTML (deploy/build_legal.py); build-only, never shipped |
