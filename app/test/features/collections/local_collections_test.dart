...
void main() {
  test(
    'saved looks are namespaced per user and never leak across a switch',
    () async {
  ...
  test(
    'closet favorites are per-user and a new account inherits none',
    () async {
      final fake = _FakeStorage(
        seed: {
          'fashionos.userA.favorites': jsonEncode(['itemA']),
        },
      );
      final c = _container(fake);
      addTearDown(c.dispose);
  ...
  test(
    'a delayed load from account A cannot populate account B state (async race)',
    () async {
