...
void main() {
  ...
  testWidgets(
    'AI Studio sheet lists the shortcuts + My Style Model coming soon',
    (tester) async {
  ...
  testWidgets('AI Looks shows the empty state when there are none', (
    tester,
  ) async {
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          generatedImagesProvider.overrideWith(
            (ref) async => <GeneratedImage>[],
          ),
        ],
    ...
    expect(
      find.text('Your AI-generated looks will appear here.'),
      findsOneWidget,
    );
  });
  ...
  testWidgets('AI Looks renders a tile per generated image', (tester) async {
    final looks = [
      ...
      GeneratedImage(
        id: 'g2',
        type: 'enhanced_item',
        outputUrl: 'https://x/2.png',
        createdAt: DateTime(2026, 6, 30),
      ),
    ];
    await tester.pumpWidget(
      ProviderScope(
        overrides: [generatedImagesProvider.overrideWith((ref) async => looks)],
        child: _app(const AiLooksScreen()),
      ),
    );
    await tester.pump();
