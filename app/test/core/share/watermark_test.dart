...
void main() {
  ...
  testWidgets('addWatermark returns a valid PNG at the source size', (
    tester,
  ) async {
    await tester.runAsync(() async {
      final src = await _solidPng(400, 600);
      final out = await addWatermark(
        src,
      ); // defaults: "Wear The Mood" + "AI Try-On"
  ...
  testWidgets('addWatermark falls back to the original bytes on bad input', (
    tester,
  ) async {
    await tester.runAsync(() async {
      ...
      expect(
        out,
        equals(junk),
      ); // never throws; returns the source so sharing works
    });
  });
}
