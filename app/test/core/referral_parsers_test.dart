...
void main() {
  group('parseReferralToken (Play install referrer)', () {
    test('extracts referral_token, ignoring organic/utm noise', () {
      expect(
        parseReferralToken(
          'referral_token=ABC123&utm_source=referral&utm_medium=share',
        ),
        'ABC123',
      );
      expect(
        parseReferralToken('utm_source=referral&referral_token=XYZ'),
        'XYZ',
      );
    });
    ...
    test('organic / empty / malformed → null (no attribution)', () {
      ...
      expect(
        parseReferralToken('utm_source=google-play&utm_medium=organic'),
        isNull,
      );
      expect(
        parseReferralToken('referral_token='),
        isNull,
      ); // present but empty
  ...
  group('referralCodeFromLink (App Link /r/<code>)', () {
    test('valid https referral links → uppercase code', () {
      expect(
        referralCodeFromLink('https://wearthemood.com/r/ab2cd3ef'),
        'AB2CD3EF',
      );
      expect(
        referralCodeFromLink('https://www.wearthemood.com/r/CODE12'),
        'CODE12',
      );
      expect(
    ...
    test('rejects wrong host / scheme / path / empty', () {
      expect(
        referralCodeFromLink('http://wearthemood.com/r/CODE'),
        isNull,
      ); // not https
      expect(
        referralCodeFromLink('https://evil.com/r/CODE'),
        isNull,
      ); // wrong host
      expect(
        referralCodeFromLink('https://wearthemood.com/about'),
        isNull,
      ); // not /r/
      expect(
        referralCodeFromLink('https://wearthemood.com/r/'),
        isNull,
      ); // no code
